...
// note: 直方图均衡化（灰度）
void CImgProcDoc::ImageHistogramEqualization(CImgProcDoc& newDoc)
{
	ASSERT(&newDoc != this);
	newDoc.Clear();
	ASSERT(this->GetColorBits() == 8);
...
// enhance: 差分增强因子
// opt: 梯度算子3x3
void CImgProcDoc::ImageSharpening(CImgProcDoc& newDoc, double reserve, double enhance,
	const std::vector<std::vector<double>>& optx, const std::vector<std::vector<double>>& opty)
{
	...
	for (LONG y = 0; y < this->bmpInfo.biHeight; ++y)
	{
		for (LONG x = 0; x < this->bmpInfo.biWidth; ++x)
		{
			// note: 如果3x3窗口中存在超出边界的像素，则不做锐化
			if (y - 1 < 0 || y + 1 >= this->bmpInfo.biHeight
				|| x - 1 < 0 || x + 1 >= this->bmpInfo.biWidth) {
				RGBQUAD rgb; this->GetPixel(x, y, &rgb);
				double val = reserve * (double)rgb.rgbRed;
				// note: curl to (BYTE)[0,hLim]
				rgb.rgbRed = rgb.rgbGreen = rgb.rgbBlue = (BYTE)min((double)hLim, max(0.0, val));
				newDoc.SetPixel(x, y, rgb);
				continue;
			}
			...
			for (int ty = 0; ty < 3; ++ty)
			{
				...
				{
					RGBQUAD trgb; this->GetPixel(x + tx - 1, y + ty - 1, &trgb);
					diffx += (double)trgb.rgbRed * (double)optx[ty][tx];
					diffy += (double)trgb.rgbRed * (double)opty[ty][tx];
				}
			}
			double detDiff = sqrt(diffx * diffx + diffy * diffy);
			RGBQUAD rgb; this->GetPixel(x, y, &rgb);
			double val = reserve * (double)rgb.rgbRed + enhance * detDiff;
...
// sigma_d: 空域方差
// sigma_r: 颜色域方差
void CImgProcDoc::ImageBilateralFiltering(CImgProcDoc& newDoc, double sigma_d, double sigma_r)
{
	ASSERT(&newDoc != this);
	newDoc.Clear();
	ASSERT(this->GetColorBits() == 8);
	ASSERT(sigma_d >= 0.0 && sigma_r >= 0.0);

	// note: 任一σ=0 时可直接拷贝
	if (sigma_d == 0.0 || sigma_r == 0.0) { newDoc = *this; return; }
	...
	DWORD radius = (DWORD)sigma_d + 1;
	DWORD size = (radius << 1) - 1;

	const double denominator_d = -1.0 / (2.0 * sigma_d * sigma_d);
	const double denominator_r = -1.0 / (2.0 * sigma_r * sigma_r);
	...
	for (LONG y = 0; y < this->bmpInfo.biHeight; ++y)
	{
		for (LONG x = 0; x < this->bmpInfo.biWidth; ++x)
		{
			...
			LONG endx = min(this->bmpInfo.biWidth, x + (LONG)radius);
			LONG height = endy - begy, width = endx - begx;
			RGBQUAD crgb; this->GetPixel(x, y, &crgb);
			...
			for (size_t ty = begy; ty < fracWeightMatrix.size() + begy; ++ty)
			{
				for (size_t tx = begx; tx < fracWeightMatrix[(size_t)ty - begy].size() + begx; ++tx)
				{
					...
					// note: 颜色域
					RGBQUAD trgb; this->GetPixel(tx, ty, &trgb);
					double clrDist = abs((double)trgb.rgbRed - (double)crgb.rgbRed);
					// note: 计算权重
					fracSumMat += fracWeightMatrix[(size_t)ty - begy][(size_t)tx - begx]
						= (dist2 > 9 * sigma_d * sigma_d) ? 0.0
						: (exp(dist2 * denominator_d) * exp(clrDist * clrDist * denominator_r));
				}
			}
			// note: 总和归一化
			for (size_t y = 0; y < fracWeightMatrix.size(); ++y)
...
// note: 增加脉冲噪声
// duty: 增加 duty% 的噪声
// binaryzation: 是否二值化（椒盐噪声）
void CImgProcDoc::ImageAddImpluseNoise(CImgProcDoc& newDoc, double dutyPercentage, bool binaryzation)
{
	ASSERT(&newDoc != this);
	newDoc.Clear();
	ASSERT(this->GetColorBits() == 8);
	ASSERT(dutyPercentage >= 0.0 && dutyPercentage <= 100.0);
	...
	if (this->GetColorBits() == 8 && this->palette.size() > 0)
	{
		hLim = (LONG)(this->palette.size() - 1);
	}

	// note: 初始化随机数引擎种子
	std::default_random_engine engine;
	engine.seed(std::random_device()());
	// note: 创建true/false二值分布，噪声插入判定
	std::bernoulli_distribution noisebd(dutyPercentage / 100.0);
	// note: 用于产生椒盐噪声
	std::bernoulli_distribution pepbd(0.5);
	// note:用于产生随机冲击噪声
	std::uniform_int_distribution<DWORD> randuid(0, hLim);
	...
	for (LONG y = 0; y < this->bmpInfo.biHeight; ++y)
	{
		for (LONG x = 0; x < this->bmpInfo.biWidth; ++x)
		{
			if (noisebd(engine))
			{
				...
				if (binaryzation)
				{
					rgb.rgbRed = rgb.rgbGreen = rgb.rgbBlue = pepbd(engine) ? 0 : (BYTE)hLim;
				}
				else
				{
					rgb.rgbRed = rgb.rgbGreen = rgb.rgbBlue = (BYTE)randuid(engine);
				}
				newDoc.SetPixel(x, y, rgb);
			}
		}
